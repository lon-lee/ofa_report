...
investing.com에서 월말 종가·환율 데이터를 수집하고, 로컬 템플릿(`template/ofa_template.xlsx`)에 채워 넣어 신고용 파일을 저장합니다.
...
```
2025년 애플 해외금융계좌신고 만들어줘. 보유주수는 1000주야.
```
...
2. 로컬 템플릿 로드 (template/ofa_template.xlsx)
3. openpyxl로 B/C/G/I 열 데이터 입력 (D/E/F 열은 수식 자동계산)
4. 완성 파일을 현재 디렉토리에 저장
```
...
예) `[애플]+해외금융계좌신고+요청자료(2025년)_최선영.xlsx`
